function [sol, norm_l1] = solver_prox_L1_full_image_no_U(x, lambda, param,i)
...
if ~isfield(param, 'rel_obj'), param.rel_obj = 1e-4; end %1e-4
...
if ~isfield(param, 'weights'), param.weights{i} = 1; end
...
psit_xA = param.Psit(param.xA{i}) ;
weights = param.weights{i};
...
dummy = soft(res, lambda*param.nu*weights);
...
sol = real(sol); 
        if i == 1
    sol(sol<param.min_x{i}) = param.min_x{i}(sol<param.min_x{i}); 
    sol_mask = sol(param.mask_app{i}) ;
         else
              min_x{i} = param.min_x{i}(param.mask_app{i});
              sol_mask = sol(param.mask_app{i}) ;
              sol_mask(sol_mask< min_x{i}) = min_x{i}(sol_mask<min_x{i});
          end
%   if i == 1
     sol_mask(sol_mask>param.max_x{i}) = param.max_x{i}(sol_mask>param.max_x{i}) ;
      sol(param.mask_app{i}) = sol_mask ;
%   else
%       sol(sol>param.max_x{i}) = param.max_x{i}(sol>param.max_x{i});
%   end
end
end

if (i == 1 || i == 2) && param.approx ~=1
