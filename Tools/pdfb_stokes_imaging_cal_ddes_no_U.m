% function [St_im, L1_v, L1_vp, L2_vv, L2_vp, rel_sol_norm_change, sol_v, snr_v, no_sub_itr_v, dual_var1, dual_var3, sol_best_bound_v, sol_reweight_v] = pdfb_par_rescaled_precond_wave_par_stokes_rw(y, epsilont, epsilonts, A, At, T, pU, W, Psi, Psit, Psiw, Psitw, Gw,stokes_P, param)
  function [St_im, param_dual, l1] = pdfb_stokes_imaging_cal_ddes_no_U(Z, stokes_P, param_l1, param,x_th, param_dual, eta_)
% 
% Z = eps_tmp;
% stokes_P = 3;
% param = param_pd;
...
g_cat = [dual_tilde{2}, -dual_tilde{1}];
St_cat = Pv_tol(g_cat, param.pol_tol);

dual_var2{1} = reshape(-St_cat(:,2),Ny,Nx);
dual_var2{2} = reshape(St_cat(:,1),Ny,Nx);
