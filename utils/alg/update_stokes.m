...
for q = 1:Jeps % around 4 s/iteration
   ...
   data_fid(q) = 0.5*norm(y(:)-y_x(:));
    ...
    if param_pd.dual_fb
        ...
        parfor i = 1:3
            x_tmp{i} = x{i} - (1.9/Ax)*real(reshape(grad_x{i},size(x{i}))); % Gradient step
             x{i} = solver_prox_L1_full_image(x_tmp{i}, eta(i)*1.9/Ax, param_l1,2, param_l1.weights{i}) ; % eta(i)*1.9/Ax
% x{i} = x_tmp{i};
        ...
        [x, param_dual,l1]  = pdfb_stokes_imaging_cal_ddes_wo_parfeval(x_tmp, 3, param_l1, param_pd, x_th,param_dual, eta_);
        for i = 1:3
            snr{i}(q) = SNR(x{i}, x_th.x{i});
        end
        l1_term(q) = l1;
...
if rem(q,200) == 0
     figure(200)
     subplot 331, imagesc(log10(abs(x_th.x{1}))), title('True I'), colorbar, axis image, axis off
     subplot 332, imagesc(log10(abs(x_th.x{2}))), title('True Q'), colorbar, axis image, axis off  
     subplot 333, imagesc(log10(abs(x_th.x{3}))), title('True U'), colorbar, axis image, axis off
     subplot 334, imagesc(log10(abs(x{1}))), title('Reconstructed I'), colorbar, axis image, axis off
     subplot 335, imagesc(log10(abs(x{2}))), title('Reconstructed Q'), colorbar, axis image, axis off
     subplot 336, imagesc(log10(abs(x{3}))), title('Reconstructed U'), colorbar, axis image, axis off
...
%             eta{i} = param_l1.eta_o(i)*weights{i}*1.9/Ax;
        ...
        for i = 1:3
            for k = 1:Ps
                ...
                weights{i,k} = param_l1.reweight_alpha(k) ./ (param_l1.reweight_alpha(k) + (d_val./d_val_max));
                weights{i,k}(d_val > d_val_max * param.reweight_abs_of_max) = 0;
                eta_{i,k} = param_l1.eta_o(i)*weights{i,k};
