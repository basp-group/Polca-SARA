function [U1, U2, epsilon, objective, time_tot, error_dirty, snr_dirty, snr_x] = joint_imaging_dde_blckCoordinate3_3(y, Y, x0, epsilon, x_th, u, v, Omega, na, T, J, K, N, S, U1, U2, scale, V, param_im, param_dde, param_algo, W, name, mu)
...
for nIter = 1:param_dde.max_it
    ...
    for nIter_dde = 1:param_dde.JUtot
        ...
        parfor t = 1:T % see parfeval or parfor for the update of U1 and U2: compute indices, then send appropriate portion of x_hat
            ...
            for a = 1:na(t)
                ...
                Ht(:, :, a) = createH1a_32(X1{t}, squeeze(V{t}(:, a, id)), U2{t}(:,id), J, S, id_nnz).';         
                id(a) = true;
            end 
            ...
            for a = 1:na(t)
                ...
                Ht(:, :, a) = createH2a_32(X2{t}, squeeze(V{t}(:, id, a)), U1{t}(:,id), J, S, id_nnz);  % [S2^2*J2*(na-1), na]
                id(a) = true;
            end 
    ...
    B_tmp = @(x) G0*so_fft2(x, K, scale);
    Bt_tmp = @(x) real(so_fft2_adj(G0'*x, N, K, scale));
    epsilon = updateEpsilon2(y, x0, epsilon, param_algo.Jeps, param_algo.tol_x, nIter, proxEpsilon, B_tmp, Bt_tmp, lambda_scale);
