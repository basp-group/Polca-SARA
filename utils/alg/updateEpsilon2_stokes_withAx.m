function [epsilon, param_dual] = updateEpsilon2_stokes_withAx(y, y0, epsilon, Jeps, tol_x, n_it, proxEpsilon, B_tmp, Bt_tmp, lambda_scale, L, Lt, M, eta, param_l1,param_pd,x_th, param_dual)
...
%-------------------------------------------------------------------------%
    ...
    Ax = 2*lambda_scale*op_norm_stokes_RIME(B_tmp,Bt_tmp, size(epsilon{1}), 1e-6, 200, 10, 3,1,L,Lt,M); % operator norm, the factor 2 comes from the complex case
% Ax = 7.660654e+06*2; %prod(512*512);
%Ax = 2.6603e+07;
...
%     [~,grad_eps] = Phit(Bt_tmp,y_eps - ytmp,2*lambda_scale);
      [~,grad_eps] = Phit(Bt_tmp,y_eps-ytmp,1,L,Lt);
...
%     l1_term(q) = (1.9/Ax)*(eta(1)*sum(abs(param_l1.Psit(epsilon{1}+x_th.x_approx{1}))) + eta(2)*sum(abs(param_l1.Psit(epsilon{2}+x_th.x_approx{2}))) + eta(3)*sum(abs(param_l1.Psit(epsilon{3}+x_th.x_approx{3}))));
    ...
    if param_pd.dual_fb
        for i = 1:3
            ...
            epsilon{i} = solver_prox_L1_full_image(eps_tmp{i}, eta(i)*1.9/Ax, param_l1,i) ;
...
%         n_itt = (n_it == 1 && q == 1);
        ...
        [epsilon, param_dual]  = pdfb_stokes_imaging_cal_ddes(eps_tmp, 3, param_l1, param_pd, x_th,param_dual, eta);
