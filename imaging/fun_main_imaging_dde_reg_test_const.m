function [x_final,snr] = fun_main_imaging_dde_reg_test_const(A_center,A,A_off,seed,im_ch,L_ch)
...
% name = [cov_type, '_', im_choice, '_', num2str(test_number),'_S=',num2str(S),'_ data_type',num2str(data_type),'_off_diag',num2str(off_diag)] ;%,'DDEs_noise'];
...
param_algo.tol_crit = 2e-3;
param_algo.tol_crit_const = 2e-2;
param_algo.iter_const = 5; 

% parameters for DDE estimation
param_dde.max_it = 20; %20; % 200;
param_dde.JUtot = 2; %2; %2; %5; % 10;
param_dde.JU1 = 5; %5;   % 5
param_dde.JU2 = 5; %5;   % 5
...
param_dde.theta_maxR(:,:,2) = A_off*ones(S2, P, 1)*sqrt(F);  %0;
param_dde.theta_maxR(:,:,3) = A_off*ones(S2, P, 1)*sqrt(F); %0;
...
%  epsilon{1} = x_th.eps_true{1};
%  epsilon{3} = x_th.eps_true{3};
...
%         Bt_tmp{i} = @(x) real(so_fft2_adj((G0_{i})'*x, N, K, sp_scale));
%     end
...
algoGS = @(par) - joint_imaging_dde_blckCoordinate6_4(y, Y, x0, ...
    epsilon, x_th, u, v, Omega, na, T, J, P, F, K, N, S, U1, ...
    U2, sp_scale, V, param_im, param_dde, param_algo, W, name2, mu,param_pd,L,Lt, B_tmp,Bt_tmp, par);
...
[U1, U2, epsilon, objective, time_dde, error_dirty, snr_dirty, snr_x] = joint_imaging_dde_blckCoordinate6_4_const(y, Y, x0, ...
    epsilon, x_th, u, v, Omega, na, T, J, P, F, K, N, S, U1, ...
...
% % figure; plot(objective(1:find(objective, 1, 'last')));
 mkdir('results')
 save(['results/joint_imaging_dde_reg_mod_pdfb_', name2,'.mat'], 'U1', 'U2', 'epsilon', ...
...
param_l1.eta_o = [30,20,20]; %[30, 1e-1, 1e-1];
end
