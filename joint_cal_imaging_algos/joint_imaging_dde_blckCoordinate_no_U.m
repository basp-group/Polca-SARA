 function [U1, U2, epsilon, objective, time_tot, error_dirty, snr_dirty, snr_x] = joint_imaging_dde_blckCoordinate_no_U(y, Y, x0, epsilon, x_th, u, v, Omega, na, T, J, P, F, K, N, S, U1, U2, scale, V, param_im, param_dde, param_algo, ...
      W, name,param_pd,L,Lt,eta1,U_th)

 
 % Joint imaging and calibration procedure with temporal
% regularization.
%-------------------------------------------------------------------------%
%%
% Input:
...
% < V       : spatial NUFFT gridding kernels with redundancy (similar to Y)
...
for i =1:3
    ...
    param_l1.xA{i} = x0{i};
    if param_pd.dual_fb == 0 || param_pd.dual_fb == 2
        for k=1:param_pd.Ps
            ...
            eta{i,k} = param_l1.eta_o(i)*lambda_scale*param_l1.weights{i,k};
...
G = cell(T, 1);
[~,D1] = computeD_reg(U1, T, [], scale_t, T);
D2 = D1;
...
for i = 1:4
    ...
    Bt_tmp{i} = @(x) (so_fft2_adj((G0_{i})'*x, N, K, scale));
end
...
for i =1:2
  ...
  b = conv_stokes_to_bright([x{1}(:),x{2}(:)],L,size(x{1}));
...
for nIter = 1:param_dde.max_it
    ...
    X1 = cell(na, 4);
    X2 = cell(na, 4);
    for a = 1:na % relatively time consuming
        id(a) = false;
        idx1 = [];
        idx2 = [];
        for t = 1:T
            ...
            idx1 = unique([idx1; idt]); 
            % for U2
            id_a = find(~isnan(squeeze(Y(id, a, t))));
            om1 = squeeze(Omega(id, a, 1, t));
            om1 = om1(id_a);
    ...
    for nIter_dde = 1:param_dde.JUtot
        ...
        %par
        for a =1:na % compute indices, then send appropriate portion of x_hat
            % update U1a
            id = true(na, 1);
            id(a) = false;
            [Ya] = create_Ya1(Y, id,a);
            Va = squeeze(V(:, a, :, :));
            Omega_a = squeeze(Omega(a,:,:,:)); 
            % build H1_a 
            ...
            [U1(:, a, :,:)] = updateUa1221_reg(Ya{a}, reshape(U1(:, a,:, :), [S2, P, 4]), ...
            ...
            for t = 1:T
                ...
                if ~isempty(id_a)
                    ...
                    [Ha(1,id_a,:,t),Ha(2,id_a,:,t),Ha(3,id_a,:,t),Ha(4,id_a,:,t),Ha(5,id_a,:,t),Ha(6,id_a,:,t),Ha(7,id_a,:,t),Ha(8,id_a,:,t)] = createH2a_32_reg(full(X2{a,1}(idt)), full(X2{a,2}(idt)), full(X2{a,3}(idt)), full(X2{a,4}(idt)), ...
                        squeeze(Va(:, id, t)), D_at, J, S, id_a);
                end
            end
            U2(:, a,:, :) = updateUa2221_reg(Ya{a}, reshape(U1(:, a, :,:), [S2, P,4]), ...
...
%    
    ...
    b_xo = conv_stokes_to_bright([x0{1}(:),x0{2}(:)],L,size(x0{1}));
    ...
    [epsilon, param_dual] = updateEpsilon2_stokes_no_U(y, Y_xo, epsilon, Jeps, tol_x, nIter, proxEpsilon, B_tmp, Bt_tmp, lambda_scale,L, Lt, M, eta, param_l1,param_pd,x_th, param_dual);
    reg = 0;
    ...
    b = conv_stokes_to_bright([x{1}(:),x{2}(:)],L,size(x{1}));
    [~, y_x] = Phi(B_tmp, b, M);
    [~, x_] = Phit_no_U(Bt_tmp, y_x, 1,L,Lt);
    [~, dirty_x] = Phit_no_U(Bt_tmp, y_x-y, 1,L,Lt);
    [~,phit_y] = Phit_no_U(Bt_tmp,y,1,L,Lt);
    ...
    objective(nIter+1) = data_fid + sum(err_temp)/2 + reg;
    % keyboard
...
%     subplot 339, plot(snr_x(:,3))
    ...
    %
    fprintf('%2s  %2s  %2s\t%2s\t%2s\t%2s  %2s  %2s\n', 'Iteration', 'Objective', 'SNR_I', 'SNR_Q', 'scaled SNR', 'Time')
    fprintf('--------------------------------------------------------------------------------\n');
    fprintf('%2i  %.2e  %.2e\t%.2e\t%.2e\t%2e  %2e  %2e\n', nIter, objective(nIter+1), snr_x(nIter+1,1), snr_x(nIter+1,2), scaled_snr, time_tot(nIter+1));
    
    fprintf('================================================================================\n');
    
    %    if mod(nIter, 5)==0
    %       n_tmp = ['_iter_',num2str(nIter)];
    %      name2 = [name, n_tmp];
    %     save(['results/img_dde_no_reg_', name2], 'U1', 'U2', 'epsilon')
    % end
    %
