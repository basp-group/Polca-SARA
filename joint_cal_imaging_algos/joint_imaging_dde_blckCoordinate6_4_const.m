function [U1, U2, epsilon, objective, time_tot, error_dirty, snr_dirty, snr_x] = joint_imaging_dde_blckCoordinate6_4_const(y, Y, x0, epsilon, x_th, u, v, Omega, na, T, J, P, F, K, N, S, U1, U2, scale, V, param_im, param_dde, param_algo, ...
...
%     [B_eps,~] = Phi(B_tmp, b_eps, M);
    ...
    [epsilon, param_dual] = updateEpsilon2_stokes(y, Y_xo, epsilon, Jeps, tol_x, flag_pd, proxEpsilon, B_tmp, Bt_tmp, lambda_scale,L, Lt, M, eta, param_l1,param_pd,x_th, param_dual);
    reg = 0;
...
fprintf('================================================================================\n');
   ...
   if ((abs((objective(nIter+1) - objective(nIter))/objective(nIter)) < param_algo.tol_crit_const) || nIter == 15) && nIter > 1 && n_Iter_change == 0

%   if nIter > param_algo.iter_const && n_Iter_change == 0
        clear eta;
            param_pd.dual_fb = 0;
            if param_pd.im_ch == 1
              param_l1.eta_o = [30,20,20];
               else
            param_l1.eta_o = [70,20,20]; %[550, 250, 250]; %[30,20,20];
            end            
        ...
        n_Iter_change = n_Iter_change +1;
        flag_pd = 1;
        flag_change = 0;
        
      
         save(['results/img_dde_const_change_', name,'.mat'], 'U1', 'U2', 'epsilon','nIter');
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % if mod(nIter, 5)==0
     %    save(['results/img_dde_no_reg_', name,'.mat'], 'U1', 'U2', 'epsilon')
     %end
%     
