 function [U1, U2, epsilon, objective, time_tot, error_dirty, snr_dirty, snr_x] = joint_imaging_dde_blckCoordinate(y, Y, x0, epsilon, x_th, u, v, Omega, na, T, J, P, F, K, N, S, U1, U2, scale, V, param_im, param_dde, param_algo, ...
      W, name, mu,param_pd,L,Lt,eta1,U_th)
...
% < S       : spatial dimension of the DDE kernels 
...
% < param_dde : DDE-related parameters (constraints definition)
...
%    
    ...
    b = conv_stokes_to_bright([x{1}(:),x{2}(:),x{3}(:)],L,size(x{1}));
    [~, y_x] = Phi(B_tmp, b, M);
    [~, x_] = Phit(Bt_tmp, y_x, 1,L,Lt);
    [~, dirty_x] = Phit(Bt_tmp, y_x-y, 1,L,Lt);
    [~,phit_y] = Phit(Bt_tmp,y,1,L,Lt);
